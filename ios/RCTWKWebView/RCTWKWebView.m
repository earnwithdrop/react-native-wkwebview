...
- (void)setupPostMessageScript {
  if (_messagingEnabled) {
    NSString *source=@"window.originalPostMessage = window.postMessage; window.postMessage = function(message, targetOrigin, transfer) { window.webkit.messageHandlers.reactNative.postMessage(message); if (typeof targetOrigin !== 'undefined') { window.originalPostMessage(message, targetOrigin, transfer); }};";
